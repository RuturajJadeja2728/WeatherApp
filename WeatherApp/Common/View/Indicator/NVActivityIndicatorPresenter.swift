...
private protocol NVActivityIndicatorPresenterState {
    func startAnimating(presenter: NVActivityIndicatorPresenter, _ fadeInAnimation: FadeInAnimation?, view: UIView?)
...
private struct NVActivityIndicatorPresenterStateAnimating: NVActivityIndicatorPresenterState {
    ...
    func stopAnimating(presenter: NVActivityIndicatorPresenter, _ fadeOutAnimation: FadeOutAnimation?) {
        ...
        DispatchQueueMain.asyncAfter(deadline: .now() + .milliseconds(activityData.minimumDisplayTime)) {
...
private struct NVActivityIndicatorPresenterStateWaitingToStop: NVActivityIndicatorPresenterState {
    func startAnimating(presenter: NVActivityIndicatorPresenter, _ fadeInAnimation: FadeInAnimation?, view: UIView?) {
        ...
        presenter.startAnimating(activityData, fadeInAnimation, view)
    }
...
private struct NVActivityIndicatorPresenterStateStopped: NVActivityIndicatorPresenterState {
    func startAnimating(presenter: NVActivityIndicatorPresenter, _ fadeInAnimation: FadeInAnimation?, view: UIView?) {
        ...
        DispatchQueueMain.asyncAfter(deadline: .now() + .milliseconds(activityData.displayTimeThreshold)) {
...
public final class NVActivityIndicatorPresenter {
    fileprivate enum State: NVActivityIndicatorPresenterState {
        ...
        func startAnimating(presenter: NVActivityIndicatorPresenter, _ fadeInAnimation: FadeInAnimation?, view: UIView?) {
            performer.startAnimating(presenter: presenter, fadeInAnimation, view: view)
        }
     ...
     */
    public final func startAnimating(_ data: ActivityData, _ fadeInAnimation: FadeInAnimation? = nil, _ view: UIView?) {
        self.data = data
        self.view = view
        state.startAnimating(presenter: self, fadeInAnimation, view: view)
    }
    ...
    public final func setMessage(_ message: String?) {
        waitingToStartGroup.notify(queue: DispatchQueueMain) {
    ...
    fileprivate func show(with activityData: ActivityData, _ fadeInAnimation: FadeInAnimation?) {
        ...
        indicatorBgView.backgroundColor = UIColor.purple.withAlphaComponent(0.65)
        indicatorBgView.roundCorners(radius: 8)
        indicatorBgView.translatesAutoresizingMaskIntoConstraints = false
        containerView.addSubview(indicatorBgView)
        
        // Add constraints for `indicatorBgView`.
        ({
            let xConstraint = NSLayoutConstraint(item: containerView, attribute: .centerX, relatedBy: .equal, toItem: indicatorBgView, attribute: .centerX, multiplier: 1, constant: 0)
            let yConstraint = NSLayoutConstraint(item: containerView, attribute: .centerY, relatedBy: .equal, toItem: indicatorBgView, attribute: .centerY, multiplier: 1, constant: 0)
            let widthConstraint = NSLayoutConstraint(item: indicatorBgView, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: activityData.size.width + 18.0)
            let heightConstraint = NSLayoutConstraint(item: indicatorBgView, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: activityData.size.height + 18.0)
...
//
        ...
        ({
            let xConstraint = NSLayoutConstraint(item: indicatorBgView, attribute: .centerX, relatedBy: .equal, toItem: activityIndicatorView, attribute: .centerX, multiplier: 1, constant: 0)
            let yConstraint = NSLayoutConstraint(item: indicatorBgView, attribute: .centerY, relatedBy: .equal, toItem: activityIndicatorView, attribute: .centerY, multiplier: 1, constant: 0)
            
            indicatorBgView.addConstraints([xConstraint, yConstraint])
        }())
        ...
        guard let keyWindow = self.view else { return }
    ...
    fileprivate func hide(_ fadeOutAnimation: FadeOutAnimation?) {
        guard let keyView = self.view else {
            return
        }
        for item in keyView.subviews
